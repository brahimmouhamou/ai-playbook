...
4. **Generate `.adp/artifacts/NNN-feature-name/prd.json`**: For each user story in the spec, create an entry:
...
- **Artifacts are per-feature.** Each feature gets its own folder under `.adp/artifacts/`. Multiple features can be in flight simultaneously.
- **prd.json is committable.** It tracks progress and should be in git. Only `progress.txt` is gitignored.
- **The spec is permanent, the plan is not.** The spec is the source of truth. If the plan drifts, regenerate from the spec.
